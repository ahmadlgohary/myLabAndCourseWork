...
public class ProceduralAbstraction {
    ...
    public static int reverseFactorial(int x){ // ex1
            double n=0;
            int iFact=1;
                if(x<=1){
                    n=1;
    ...
    public static void main(String[] args){
        System.out.println("type 'revFact' to run revFactorial main method ");
        System.out.println("type 'matrix' to run isMatrixNice main method ");
        String ans1 = user.nextLine().trim().toLowerCase();
       
        if(ans1.equals("revfact")){
            System.out.println(numbers.reverseFactorial(121));     // test a number of my own   
            System.out.println("Do you want to try your own number? yes/no");    
            String ans2 = user.nextLine().trim().toLowerCase();
        ...
        else if(ans1.equals("matrix")){
        int[][] a = new int[][] //testing the 2 matrixs given in the manual
           {{ 2 , 7, 6 },
            { 9, 5, 1 },
            {4 , 3, 8 }};
        for (int i=0; i<3; ++i){
